...
<div class="flex items-center justify-between flex-wrap gap-3">
  <p class="text-sm text-ink-soft max-w-xl" data-en="Levels live inside a track (e.g. Grade 1 inside the Children's Track, or ቀዳማይ inside Youth & Adult). Sort order controls how they appear in lists." data-am="ደረጃዎች በኮርስ ውስጥ ይገኛሉ።">Levels live inside a track. Sort order controls how they appear in lists.</p>
...
<section id="formPanel" class="panel hidden">
  ...
  <form id="entityForm" class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <div>
      <label class="block text-[11px] font-semibold uppercase tracking-widestest text-ink-soft mb-2">Track</label>
      <select id="f_track" class="input-field" required></select>
    </div>
    <div>
      <label class="block text-[11px] font-semibold uppercase tracking-widestest text-ink-soft mb-2">Level name</label>
      <input id="f_name" class="input-field" required placeholder="e.g. Grade 1 or ቀዳማይ" />
    </div>
    <div>
      <label class="block text-[11px] font-semibold uppercase tracking-widestest text-ink-soft mb-2">Sort order</label>
...
<section class="panel">
  <header class="px-6 py-5 border-b border-outline-soft/40 flex items-center justify-between">
    <h2 class="font-display text-lg text-ink">All levels · <span id="rowCount" class="text-ink-soft text-sm">—</span></h2>
  </header>
  <div class="table-wrap">
    <table class="data">
      <thead><tr><th>Track</th><th>Name</th><th>Sort</th><th>Status</th><th class="text-right">&nbsp;</th></tr></thead>
...
<script>
  ...
  function showForm(item) {
    ...
    document.getElementById('f_sort').value = item ? item.sort_order : '0';
    document.getElementById('f_track').value = item ? item.track_id : '';
    formTitle.textContent = item ? 'Edit Level' : 'New Level';
  ...
  function render() {
    ...
    if (!all.length) { tbody.innerHTML = '<tr><td colspan="5" class="text-center text-ink-soft py-16">No levels yet.</td></tr>'; return; }
    tbody.innerHTML = all.map(function (l) {
      var pill = l.is_archived == 1 ? '<span class="pill pill-archived">Archived</span>' : '<span class="pill pill-active">Active</span>';
      return '<tr>' +
        '<td class="text-ink-soft">'+escHtml(l.track_name)+'</td>' +
  ...
  function fillTrackSelect() {
    var sel = document.getElementById('f_track');
    sel.innerHTML = '<option value="">Choose a track…</option>' + tracks.filter(function (t) { return t.is_archived == 0; }).map(function (t) {
  ...
  async function load() {
    try {
      var t = await gs.api('/api/admin/tracks/index.php');
      tracks = t.data || []; fillTrackSelect();
      var d = await gs.api('/api/admin/levels/index.php');
      all = d.data || []; render();
  ...
  document.getElementById('entityForm').addEventListener('submit', async function (e) {
    ...
    var body = {
      track_id: parseInt(document.getElementById('f_track').value, 10),
  ...
  document.addEventListener('click', async function (e) {
    ...
    if (t.dataset.edit) {
      ...
      if (!await gs.confirm('Archive this level?')) return;
